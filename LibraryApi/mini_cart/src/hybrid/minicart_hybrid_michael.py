...
class MiniCartHybridLibrary:
    ...
    def get_keyword_names(self) -> list[str]:
        ...
        if self._group == "all":
            fn_names: list[str] = [
                ...
                if not name.startswith("_") and \
                    callable(getattr(self._target, name, None))
            ]
        # 2) Get only the keywords that are in the selected group.
        elif self._group in _GROUPS:
            fn_names: list[str] = list(_GROUPS[self._group])
        ...
        return [self._to_robot_name(n) for n in fn_names]
    ...
    def __getattr__(self, name: str) -> Callable:
        ...
        fn_name = self._from_robot_name(name)
        # 1) First: local method on this library class
        try:
            local = object.__getattribute__(self, fn_name)
            if callable(local):
                return local
        except AttributeError:
            pass
        # Try to get the attribute (by name) from the module.
        func = getattr(self._target, fn_name, None)
        # Return if callable and public.
        if callable(func) and not fn_name.startswith("_"):
            return func
    ...
    def library_info(self) -> str:
        """
        This method is defined directly on the hybrid class.
        """
        return f"MiniCartHybridLibrary (group='{self._group}')"
